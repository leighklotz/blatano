...
void test_crc_one(uint32_t n, uint32_t expected) {
  uint32_t nhash = get_crc32c(n);
  if (nhash == expected)  {
    DPRINTF("[OK] 0x%x hash 0x%x 0x%x\n", n, nhash, expected);
  } else {
...
//   assert_equal(0, "voxhomuxist", robot_named_n(buf, 0));
//   assert_equal(1, "numakutron", robot_named_n(buf, 1));
//   assert_equal(1000000, "purwersen", robot_named_n(buf, 1000000));
//   assert_equal(0x7ffffffe, "sixwergur", robot_named_n(buf, 0x7ffffffe));
//   assert_equal(0x7fffffff, "yoxkonunoron", robot_named_n(buf, 0x7fffffff));
//   assert_equal(0x80000000, "linhayunin", robot_named_n(buf, 0x80000000));
//   assert_equal(0x80000001, "werlinoxivor", robot_named_n(buf, 0x80000001));
//   assert_equal(big-1, "bixgirror", robot_named_n(buf, big-1));
//   assert_equal(big, "goxyokaist", robot_named_n(buf, big));
// }
